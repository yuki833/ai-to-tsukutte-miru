---
title: "AIと価格・利益診断サービスを作った全過程：調査、プロンプト、開発、公開の失敗"
description: "無料の価格・利益診断サービスを、AIと人間がどう分担して作ったか。調査、主なプロンプト、実装、Cloudflare公開、プライバシー上の失敗まで記録します。"
...
この記事は、無料の価格・利益診断サービス「余白 / Yohaku」の**制作過程編**です。AIと人がどう分けて作ったのかを、そのままのこしています。

なぜ価格と利益をテーマにしたのか、どんな困りごとを解こうとしたのかは、先に[課題解決編：小さな商売の値付けが難しい理由](/articles/why-small-business-pricing-is-hard/)に書きました。国や役所が出している資料をもとに、困りごとを細かく分けています。

「AIについて書くブログ」から、もう一歩進めることにしました。

AIでものを作り、世の中へ出し、使われ方を見ながら直していく。その流れをブログにのこす試みです。1つ目として、ひとり商売向けの価格・利益診断サービス「余白 / Yohaku」を作りました。

[余白を使ってみる](https://yohaku-price-check.yohaku-tools.workers.dev/?src=blog)

この記事では、完成品だけでなく、なぜこのテーマを選んだのか、どんな指示をAIへ出したのか、人間は何をしたのか、公開時に何を失敗したのかまで残します。

## 今回の実験条件

![人間が出した2つの条件と、そこから決めたこと。「AIだけでほぼ自走できるもの」から物販ではなくWebツールを選び、「無料で始められて学習コストも低いもの」から有料販売を目指さず無料公開を先にした](/images/articles/yohaku-business/05-experiment-conditions.svg)
...
![無料、登録不要、在庫なし、AIで改善しやすいという4条件から価格・利益診断を選んだ](/images/articles/yohaku-business/01-why-yohaku.svg)
...
どんな困りごとなのか、その裏づけは[課題解決編](/articles/why-small-business-pricing-is-hard/)のほうへ分けました。ここでは、何と何を比べて、どんな中身にすると決めたのかだけを書きます。

事前調査では、損益分岐点や時給を計算する無料ツールはすでに複数ありました。つまり「計算できる」だけでは差別化になりません。

そこで考えを変えました。計算だけをする道具ではなく、**6つの数字を1つの画面に入れると、次にどうするかを決める材料がまとめて返ってくる**ものにしました。入れてもらうのは、販売価格、1件あたりの経費、作業時間、固定費、販売見込み数、目標利益です。
...
![販売価格・1件あたりの経費・作業時間・固定費・販売見込み数・目標利益の6つを入れると、1件あたりの利益・利益率・実質時給・月間利益・黒字化に必要な販売数・目標利益に必要な販売数・10%値上げの効果の7つが返る。計算はブラウザーの中で、入れた数字はサーバーへ送らない](/images/articles/yohaku-business/06-what-it-returns.svg)
...
![AIが調査・制作・計測・改善を担い、人間が目的・権限・公開判断を担う役割分担](/images/articles/yohaku-business/02-ai-human-roles.svg)
...
![AIがデータベース・設定・ビルドまで準備し、公開コマンドの認証と通信の制限で止まり、人間が端末でコマンドを2つ打ち、そのあとAIへ戻して本番URLと保存されたイベントを確認した、という4段の流れ](/images/articles/yohaku-business/07-handover-at-deploy.svg)
...
![公開、匿名計測、問題発見、小さな改善、再計測を繰り返し、30日後に事業判断する](/images/articles/yohaku-business/03-growth-loop.svg)
...
会議のあとに議事録を書く手間を減らしたいなら、[議事録をAIに書かせる。コピーして貼るだけの手順](/articles/ai-meeting-minutes/)です。そのまま貼って使える指示文を1つだけ載せています。

社内の資料をAIへ貼ってよいか迷うなら、[社内の資料をAIに貼っていいのか。社名と金額を伏せて貼り、答えを元に戻す手順](/articles/ai-confidential-masking/)を読んでください。この記事で書いた「公開URLに個人ハンドルが入った」失敗と、同じ種類の見落としを扱っています。
